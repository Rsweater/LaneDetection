    ...
    def loss(self, out_dict, img_metas):
        ...
        for stage in range(self.refine_layers):
            for b, img_meta in enumerate(img_metas):
                ...
                lane_ys = prior_ys.repeat(len(matched_col_inds)).view(-1, 1)
                pred_lane = torch.cat(
                    (pred_xs.clone().reshape(-1, 1) * (self.img_w - 1) / self.img_w, lane_ys), dim=1
                )
                target_lane = torch.cat(
                    (target_xs.clone().reshape(-1, 1) / self.img_w, lane_ys), dim=1
                )

                dist_loss = dist_loss + self.loss_dist(
                    pred_lane, target_lane,
                )

        cls_loss /= batch_size * self.refine_layers
        dist_loss /= batch_size * self.refine_layers
